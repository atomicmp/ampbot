...
async function updateFactionInformation(role) {
  await db('factions').where('role_id', role.id).update({
...
module.exports = async function factionInitCommand(msg) {
  ...
  for (const faction of factionsList) {
    const factionRole = msg.guild.roles.find(role => role.id == faction.role_id);
    await updateFactionInformation(factionRole);
    await db('users').where('faction', faction.faction_id).update({faction: null})

    const factionMembers = factionRole.members.array();
    await Promise.all(
      factionMembers.map(member =>
        db('users').where('discord_id', member.user.id).update({faction: faction.faction_id}))
    )
  }
